...
void sdhci_send_command(struct sdhci_host *host, struct mmc_command *cmd)
{
	...
	while (sdhci_readl(host, SDHCI_PRESENT_STATE) & mask) {
		if (timeout == 0) {
			pr_debug("BBox;%s: Controller never released "
				"inhibit bit(s).\n", mmc_hostname(host->mmc));
			MMC_TRACE(host->mmc,
...
static void sdhci_timeout_timer(unsigned long data)
{
	...
	if (host->mrq) {
		pr_debug("BBox;%s: Timeout waiting for hardware "
			"interrupt.\n", mmc_hostname(host->mmc));
...
static void sdhci_cmd_irq(struct sdhci_host *host, u32 intmask, u32 *mask)
{
	...
	if (!host->cmd) {
		pr_debug("BBox;%s: Got command interrupt 0x%08x even "
...
//	trace_mmc_data_rw_end(command, intmask);
	...
	if (!host->data) {
		...
		pr_debug("BBox;%s: Got data interrupt 0x%08x even "
...
out:
	...
	if (unexpected) {
		pr_debug("BBox;%s: Unexpected interrupt 0x%08x.\n",
